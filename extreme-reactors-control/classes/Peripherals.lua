...
local function searchPeripherals()
    ...
    for i = 1, #peripheralList do
        ...
        elseif periType == "modem" then
            ...
            local foundGetEnergy = peri.getEnergyStored or peri.getEnergy
            ...
            local isBase = (not isMekanism and not isThermalExpansion and not isDraconicEvolution) and foundGetEnergy
...
function _G.checkPeripherals()
    ...
    _G.monitorSizeX = monX
    _G.monitorSizeY = monY

    if amountReactors < 1 then           
        -- do no check monitor is controlled by user for wireless stats 
    else
        debugOutput("Monitor Size is  x -> " ..monX.. " y-> "..monY)
        if _G.monitorSizeX < 70 or _G.monitorSizeY < 26 then
