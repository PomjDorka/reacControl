...
function createAllButtons()
    if _G.smallMonitor == 0 then
        ...
        local xStart = 32
        local xEnd = _G.monitorSizeX - 7;
        local columnsLimit = math.floor((xEnd -xStart) / 7) + 1
        local rowLimit = (_G.monitorSizeY - 4) / 2
        local currentColumn = 0
        local currentRow = 0
        --debugOutput("Column Limit: " ..columnsLimit .. " Based off of ((".. xStart .. " - " ..xEnd.. ") /7) +1")
            
        --Turbine buttons
        for i = 0, amountTurbines, 1 do
            local x = xStart + (currentColumn * 7)
            local y = 4 + (currentRow * 2)

            if i < 6 then
                debugOutput("new turbine Button (:".. (i).."): col-> "..currentColumn.." x -> "..x.. " y-> "..y)
            end

            if x+5 > _G.monitorSizeX or y > _G.monitorSizeY then
                --do nothing
            else       
                --add buttons           
                if overallMode == "auto" then           
                    page:add("#" .. (i + 1), function() printStatsAuto(i) end, x, y, x + 5, y)
                elseif overallMode == "manual" then                
                    page:add("#" .. (i + 1), function() printStatsMan(i) end, x, y, x + 5, y)
                end --mode
            end

            currentColumn = currentColumn + 1

            if currentColumn >= columnsLimit then
                --debugOutput("next row: currentColumn -> "..currentColumn.. " limit -> "..columnsLimit)
...
function printStatsAuto(turbine)
    ...
    local production, productionString = input.tightenRFNumber(data.rfProduced)
    
    controlMonitor.write(
        _G.language:getText("rfProduction")..
            (input.formatNumberComma(production)) .. " "..productionString .."/t")
    ...
    controlMonitor.write(
        _G.language:getText("rfProduction")..
            (input.formatNumberComma(data.efficiency)) .. " "..productionString .."/mb")
    ...
    local production, productionString = input.tightenRFNumber(turbines[turbine]:energyProduction())
    controlMonitor.write(
        _G.language:getText("rfProduction")..
            (input.formatNumberComma(production)) .. " "..productionString .."/t")
    ...
    local production, productionString = input.tightenRFNumber(getTurbineEnergy(turbine))
    
    controlMonitor.write(input.formatNumberComma(math.floor(production)) .. " " .. productionString)

    --prints the current program version
    controlMonitor.setCursorPos(2, _G.monitorSizeY - 1)
